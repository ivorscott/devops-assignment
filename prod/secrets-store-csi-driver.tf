resource "helm_release" "secrets_csi_driver" {
  ...
  # Required when using ENV variables
  set = [
    {
...
data "aws_iam_policy_document" "myapp_secrets" {
  statement {
    ...
    condition {
      ...
      values   = ["system:serviceaccount:devpies:myapp"]
    }
    ...
    Statement = [
      {
        ...
        Resource = "*" # Always use a specific secret arn - "arn:*:secretsmanager:*:*:secret:my-secret-kkargS"
      }
    ]
  })
}
